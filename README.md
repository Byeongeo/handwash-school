...
4. `샘플 세트 이름`을 입력합니다(예: `3학년_보건실`). 세트 이름·기기 메모·현재 라벨은 기기에 자동 저장됩니다.
5. 카메라 위 툴바에서 라벨을 고르고 `수집 시작`을 누르면 동작 유지 중 자동 저장되어 **100개가 되면
   음성 안내와 함께 저절로 완료**됩니다(약 30초 · 버튼에 n/100 표시 · 재누름으로 조기 종료).
   같은 라벨을 더 모으려면 다시 `수집 시작`(100개 단위 추가).
